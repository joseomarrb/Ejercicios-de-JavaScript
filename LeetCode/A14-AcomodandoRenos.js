 ...
 * Hay que mover cada reno, desde su posición actual, hasta un establo. Pero hay que tener en 
 * cuenta que sólo puede haber un reno por establo.
 * 
 * Tu tarea es calcular el mínimo número de movimientos necesarios para que todos los renos acaben en un establo.
 * 
 * Nota: Ten en cuenta que el array de establos siempre tendrá el mismo tamaño que el array de 
 * renos y que siempre los establos serán únicos.
 */
...
function minMovesToStables(reindeer, stables) {
    ...
    for (let i = 0; i < reindeer.length && i < stables.length; i++) {
        if (reindeer[i] === stables[i]) {
            output += 0;
        } else {
            output += Math.abs(reindeer[i] - stables[i])
        }
    }
